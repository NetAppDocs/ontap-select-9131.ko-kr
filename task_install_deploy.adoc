...
.이 작업에 대해
ONTAP Select Deploy 관리 유틸리티는 OVF(Open Virtualization Format) 표준을 기반으로 가상 머신으로 패키징됩니다. 단일 압축 파일에는 접미사가 있습니다 `ova`.

.단계
. 웹 브라우저를 사용하여 NetApp Support 사이트에 액세스하고 * 빠른 링크 * 를 클릭합니다.
